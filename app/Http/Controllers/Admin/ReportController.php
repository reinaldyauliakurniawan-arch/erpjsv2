...
class ReportController extends Controller
{
    ...
    {
        $p = $this->reports->resolvePeriod($request->get('period', 'month'), $request->get('from'), $request->get('to'));
        $from = $p['from'];
        $to = $p['to'];
        $period = $p['period'];

        // Rumus arus kas dari sumber tunggal (dipakai identik oleh Dashboard CFO).
        $cf = $this->reports->cashFlow($from, $to);

        return view('admin.reports.cash_flow', array_merge($cf, compact('from', 'to', 'period')));
    }
    ...
    public function trialBalance(Request $request)
    {
        ...
        if ($request->get('from')) {
            $query->whereDate('journals.date', '>=', $request->get('from'));
        }
        if ($request->get('to')) {
            $query->whereDate('journals.date', '<=', $request->get('to'));
        }
    ...
    public function profitLoss(Request $request)
    {
        ...
        return view('admin.reports.profit_loss', array_merge($pl, compact('from', 'to', 'period')));
    }
    ...
    public function balanceSheet(Request $request)
    {
        ...
        // Rumus neraca dari sumber tunggal (dipakai identik oleh Dashboard CFO).
    ...
    public function deferredRevenue(Request $request)
    {
        ...
        if ($filterProgram) {
            $query->where('programs.id', $filterProgram);
        }
        if ($filterFrom) {
            $query->whereDate('enrollments.created_at', '>=', $filterFrom);
        }
        if ($filterTo) {
            $query->whereDate('enrollments.created_at', '<=', $filterTo);
        }
        ...
        $enrollments = $query->get()->map(function ($e) {
            ...
            if ($e->total_meetings <= 0 || $paidAmount <= 0) {
                return null;
            }
    ...
    public function storeOpeningBalance(Request $request)
    {
        ...
        $accountMap = Account::whereIn('id', $balances->keys())->pluck('code', 'id');
        ...
        if (Journal::where('reference', $obReference)->exists()) {
