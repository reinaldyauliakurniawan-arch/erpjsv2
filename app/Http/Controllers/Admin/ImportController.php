        ...
        try {
            DB::transaction(function () use ($data, &$imported, &$errors) {
                foreach ($data as $index => $row) {
                    ...
                    if (!in_array($type, $validTypes)) { $errors[] = "Row {$index}: type '{$row[2]}' tidak valid."; continue; }
        ...
        try {
            DB::transaction(function () use ($data, &$imported, &$errors) {
                foreach ($data as $index => $row) {
                    ...
                    if (!is_numeric($cost)) { $errors[] = "Row {$index}: cost harus angka, dapat '{$cost}'."; continue; }
                    if (!is_numeric($salvageValue)) { $errors[] = "Row {$index}: salvage_value harus angka, dapat '{$salvageValue}'."; continue; }
                    if (!is_numeric($usefulLife)) { $errors[] = "Row {$index}: useful_life harus angka, dapat '{$usefulLife}'."; continue; }
