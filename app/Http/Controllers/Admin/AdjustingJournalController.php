...
class AdjustingJournalController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $totalDebit = (float) collect($request->items)->sum('debit');
        $totalCredit = (float) collect($request->items)->sum('credit');

        // Toleransi seperti AccountingService::createJournal (jangan pakai
        // !== di float — bisa gagal untuk nilai yang sebenarnya sama karena
        // representasi biner, atau lolos untuk selisih kecil yang salah).
        if (abs($totalDebit - $totalCredit) > 0.01) {
            return back()->withErrors(['items' => 'Total debit harus sama dengan total kredit.']);
        }

        $accountCodes = Account::whereIn('id', collect($request->items)->pluck('account_id'))->pluck('code', 'id');

        try {
            DB::transaction(function () use ($request, $accountCodes) {
                ...
                // Posting sesungguhnya ke buku besar SELALU lewat AccountingService
                // — satu-satunya tempat yang memvalidasi balance & idempotency
                // dengan row lock (bukan duplikasi logic di sini).
                $journal = $this->accounting->createJournal(
                    $aj->period,
                    "[AJP] {$aj->description}",
                    $reference,
                    collect($request->items)->map(fn ($item) => [
                        'account_code' => $accountCodes[$item['account_id']],
                        'debit' => $item['debit'],
                        'credit' => $item['credit'],
                    ])->all(),
                    'adjusting'
        ...
        } catch (BalanceMismatchException|IdempotencyException|AccountNotFoundException $e) {
